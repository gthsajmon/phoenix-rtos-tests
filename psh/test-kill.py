...
def create_psh_processes(p, count):
    ...
    for proc in get_process_list(p):
        task = proc.get('task')
        if proc.get('state') == 'sleep' and (task == 'psh' or task == '/bin/psh'):
...
def assert_kill_procs(p, pid_list, pid_count):
    ...
    for proc in get_process_list(p):
        assert not proc['pid'] in pid_list, \
            f'The process with id: {proc["pid"]} is still in the processes list, even though it has been killed'
