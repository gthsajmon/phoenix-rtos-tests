...
def assert_text_files(p):
    exp_regex = r'psh: /etc/passwd execution halted with error code -(\d+)(\r+)\n'
    msg = 'Wrong error message when executing text file'
    psh.assert_cmd(p, exp_regex, msg, is_regex=True)

    msg = 'Creating testfile by touch command failed'
    psh.assert_cmd(p, 'touch testfile', '', msg)

    msg = 'Wrong error message when executing empty file'
    psh.assert_cmd(p, '/testfile', exp_regex, msg, is_regex=True)
...
def harness(p):
    ...
    # assert_devices(p)
    # waiting for https://github.com/phoenix-rtos/phoenix-rtos-utils/pull/112 merge
    # assert_text_files(p)
